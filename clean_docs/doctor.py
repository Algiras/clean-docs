...
class Doctor:
    ...
    def _check_github_token(self) -> None:
        ...
        if token:
            ...
            self.checks.append(
                ("GITHUB_TOKEN", False, "Not set (needed if gh CLI not authenticated)")
            )
    ...
    def _check_semantic_deps(self) -> None:
        ...
        try:
            ...
            self.checks.append(
                ("Semantic Analysis (Optional)", True, "sentence-transformers available")
            )
        except ImportError:
            self.checks.append(
                (
    ...
    def _check_copilot_cli(self) -> None:
        ...
        try:
            ...
            if result.returncode == 0:
                self.checks.append(
                    ("Copilot CLI (Optional)", True, "Available for agent-based fixes")
                )
            else:
                self.checks.append(
                    (
            ...
            self.checks.append(
                ("Copilot CLI (Optional)", False, "Not installed (optional for agent mode)")
            )
