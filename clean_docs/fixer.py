...
class LinkFixer:
    ...
    async def fix_iteratively(
        ...
        while iteration < self.max_iterations:
            ...
            if total_broken == 0:
                self.console.print(f"[green]✓ Iteration {iteration}: No broken links found![/green]")
                break
            ...
            if not auto_fixable:
                self.console.print(f"[yellow]Iteration {iteration}: {total_broken} broken links, but none are auto-fixable[/yellow]")
                break
            ...
            self.console.print(f"[green]Applied {succeeded}/{attempted} fixes in iteration {iteration}[/green]")
