...
class EmbeddingManager:
    ...
    def embed_code_file(self, file_path: Path) -> EmbeddingEntry:
        ...
        # Generate embeddings
        if self.console:
            self.console.print(f"[dim]Embedding code {file_path.name} ({len(chunks)} chunks)...[/dim]")
...
class SemanticAnalyzer:
    ...
    def find_orphaned_docs(self, threshold: float = 0.5) -> List[Tuple[EmbeddingEntry, List[Tuple[EmbeddingEntry, float]]]]:
    ...
    def find_missing_docs(self, threshold: float = 0.5) -> List[Tuple[EmbeddingEntry, List[Tuple[EmbeddingEntry, float]]]]:
    ...
    def suggest_related_code(self, doc_path: Path, top_k: int = 5) -> List[Tuple[EmbeddingEntry, float]]:
